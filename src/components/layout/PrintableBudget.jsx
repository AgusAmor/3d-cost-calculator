            ...
            <React.Fragment key={plate.id}>
                ...
                <tr>
                  <td className="py-3 pr-4">
                    ...
                    <div className="text-xs text-slate-500">
                      Filamento: {plate.selectedFilamentName} (+10% scrap)
                    </div>
                  </td>
                  <td className="py-3 text-right font-mono">
                    {plate.filamentGrams} g
                  </td>
                  <td className="py-3 text-right font-mono">
                    {formatCurrency(
                      plate.filamentCost / (plate.filamentGrams || 1),
                    )}
                    /g
                  </td>
                  <td className="py-3 text-right font-semibold font-mono">
                    {formatCurrency(plate.filamentCost)}
                  </td>
                </tr>
              )}
              {/* Time row */}
              {plate.minutes > 0 && (
                <tr>
                  <td className="py-3 pr-4">
                    <div className="font-medium text-slate-900">
                      {plate.name} (Tiempo de Impresión)
                    </div>
                    <div className="text-xs text-slate-500">
                      Depreciación de impresora y consumo eléctrico (+10% scrap)
                    </div>
                  </td>
                  <td className="py-3 text-right font-mono">
                  ...
                  <td className="py-3 text-right font-mono">
                    {formatCurrency(plate.timeCost / (plate.decimalHours || 1))}
                    /h
                  </td>
                  <td className="py-3 text-right font-semibold font-mono">
                    {formatCurrency(plate.timeCost)}
                  </td>
                </tr>
              )}
            </React.Fragment>
          ...
          {project.laborRatePerHour > 0 && results.totalDecimalHours > 0 && (
            <tr>
              <td className="py-3 pr-4">
                ...
                <div className="text-xs text-slate-500">
                  Costo total de operación y supervisión
                </div>
              </td>
              <td className="py-3 text-right font-mono">
