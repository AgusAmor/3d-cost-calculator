    ...
    <div className="hidden print:block bg-white text-slate-900 p-8 min-h-screen font-sans w-full max-w-200 mx-auto">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 mb-8">
        ...
        <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm">
          ...
          <div>
            <span className="text-slate-500">Filamento:</span>{" "}
            <span className="font-semibold text-slate-900">
              {results.selectedFilamentName}
            </span>
          </div>
          <div>
          ...
          <div>
            <span className="text-slate-500">Tiempo de Impresión:</span>{" "}
            <span className="font-mono font-semibold text-slate-900">
      ...
      <table className="w-full text-left text-sm border-collapse mb-8">
        ...
        <tbody className="divide-y divide-slate-200">
          <tr>
            <td className="py-3 pr-4">
              ...
              <div className="text-xs text-slate-500">
                Consumo de material con 10% scrap incluido
              </div>
            </td>
            ...
            <td className="py-3 text-right font-mono">
              {formatCurrency(
                results.filamentCost / (project.filamentGrams || 1),
              )}
              /g
            </td>
          ...
          <tr>
            <td className="py-3 pr-4">
              <div className="font-medium text-slate-900">
                Costo Operativo de Máquina (Tiempo)
              </div>
              <div className="text-xs text-slate-500">
                Consumo eléctrico ({settings.printerPower}W) y depreciación de
                hardware (${settings.printerWearRate}/h) + 10% scrap
              </div>
            </td>
            <td className="py-3 text-right font-mono">
              {results.decimalHours.toFixed(2)} hs
            </td>
            <td className="py-3 text-right font-mono">
              {formatCurrency(results.timeCost / (results.decimalHours || 1))}/h
            </td>
            <td className="py-3 text-right font-semibold font-mono">
              {formatCurrency(results.timeCost)}
            </td>
          </tr>
          {project.laborRatePerHour > 0 && (
            <tr>
              <td className="py-3 pr-4">
                <div className="font-medium text-slate-900">
                ...
                <div className="text-xs text-slate-500">
                  Costo de operario asignado al proyecto
                </div>
              </td>
              <td className="py-3 text-right font-mono">
                {results.decimalHours.toFixed(2)} hs
              ...
              <td className="py-3 text-right font-mono">
                {formatCurrency(project.laborRatePerHour)}/h
              </td>
              <td className="py-3 text-right font-semibold font-mono">
          ...
          {project.otherCosts > 0 && (
            <tr>
              <td className="py-3 pr-4">
                ...
                <div className="text-xs text-slate-500">
                  Costo fijo añadido al presupuesto
                </div>
              </td>
              <td className="py-3 text-right">-</td>
              <td className="py-3 text-right font-mono">
                {formatCurrency(project.otherCosts)}
              </td>
              <td className="py-3 text-right font-semibold font-mono">
                {formatCurrency(project.otherCosts)}
              </td>
        ...
        <div className="w-75 space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-mono text-slate-900">
              {formatCurrency(results.productionCost)}
            </span>
          </div>
          {results.laborCost > 0 && (
            <div className="flex justify-between text-sm">
      ...
      <div className="mt-16 text-center text-xs text-slate-400 border-t border-slate-100 pt-6">
        ...
        <p className="mt-1">
          © {new Date().getFullYear()} AgusAmor. Todos los derechos reservados.
        </p>
      </div>
    </div>
  );
